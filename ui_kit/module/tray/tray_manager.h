...
namespace nim_comp
{
	typedef std::function<void()> TrayLeftClick;
	typedef std::function<void()> TrayRightClick;
	typedef std::function<void()> TrayDoubleClick;

	enum TrayEventType
	{
		TrayEventType_LeftClick,
		TrayEventType_RightClick,
		TrayEventType_DoubleClick
	};

	class TrayManager
	{
	public:
		SINGLETON_DEFINE(TrayManager);
		TrayManager();
		virtual ~TrayManager();
	...
	public:
		...
		*/
		...
		void RegEventCallback(TrayLeftClick ev, TrayEventType type);
	private:
		/**
		* 处理内部窗口消息
		* @param[in] hWnd 窗口句柄
	...
	private:
		...
		std::vector<TrayLeftClick>		left_click_cbs_;
		std::vector<TrayRightClick>		right_click_cbs_;
		std::vector<TrayDoubleClick>	double_click_cbs_;
	};

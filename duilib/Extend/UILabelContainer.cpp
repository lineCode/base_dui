...
namespace dui{
	...
	LPCTSTR LabelContainer::GetClass() const
	{
		return DUI_CTR_LABELCONTAINER;
	}

	LPVOID LabelContainer::GetInterface(LPCTSTR pstrName)
	{
		if (_tcscmp(pstrName, DUI_CTR_LABELCONTAINER) == 0) return static_cast<LabelContainer*>(this);
		return __super::GetInterface(pstrName);
	}

	void LabelContainer::SetFixedWidth(int cx)
	{
		m_bNeedEstimateSize = true;
		Control::SetFixedWidth(cx);
	}

	void LabelContainer::SetFixedHeight(int cy)
	{
		m_bNeedEstimateSize = true;
		Control::SetFixedHeight(cy);
	}

	void LabelContainer::SetText(LPCTSTR pstrText)
	{
	...
	void LabelContainer::SetTextStyle(UINT uStyle)
	{
		m_uTextStyle = uStyle;
		m_bNeedEstimateSize = true;
		Invalidate();
	...
	UINT LabelContainer::GetTextStyle() const
	{
		return m_uTextStyle;
	}

	bool LabelContainer::IsMultiLine()
	{
		return (m_uTextStyle & DT_SINGLELINE) == 0;
	}

	void LabelContainer::SetMultiLine(bool bMultiLine)
	{
		if (bMultiLine)	{
	...
	void LabelContainer::SetTextColor(DWORD dwTextColor)
	{
		m_dwTextColor = dwTextColor;
		Invalidate();
	}

	DWORD LabelContainer::GetTextColor() const
	{
		return m_dwTextColor;
	}

	void LabelContainer::SetDisabledTextColor(DWORD dwTextColor)
	{
		m_dwDisabledTextColor = dwTextColor;
		Invalidate();
	}

	DWORD LabelContainer::GetDisabledTextColor() const
	{
		return m_dwDisabledTextColor;
	}

	void LabelContainer::SetFont(int index)
	{
		m_iFont = index;
		m_bNeedEstimateSize = true;
		Invalidate();
	...
	int LabelContainer::GetFont() const
	{
		return m_iFont;
	}

	RECT LabelContainer::GetTextPadding() const
	{
		if (m_pManager) return m_pManager->GetDPIObj()->Scale(m_rcTextPadding);
		return m_rcTextPadding;
	}

	void LabelContainer::SetTextPadding(RECT rc)
	{
		m_rcTextPadding = rc;
		m_bNeedEstimateSize = true;
		Invalidate();
	...
	bool LabelContainer::IsShowHtml()
	{
		return m_bShowHtml;
	}

	void LabelContainer::SetShowHtml(bool bShowHtml)
	{
	...
	SIZE LabelContainer::EstimateSize(SIZE szAvailable)
	{
		if (m_cxyFixed.cx > 0 && m_cxyFixed.cy > 0) return Control::EstimateSize(szAvailable);
		...
		if (m_bNeedEstimateSize) {
			...
			if ((m_uTextStyle & DT_SINGLELINE) != 0) {
				...
				if (m_cxyFixedLast.cx == 0) {
					...
					if (m_bShowHtml) {
						int nLinks = 0;
						CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText.c_str(), 0, NULL, NULL, nLinks, m_iFont, DT_CALCRECT | m_uTextStyle & ~DT_RIGHT & ~DT_CENTER);
					}
					else {
						CRenderEngine::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText.c_str(), 0, m_iFont, DT_CALCRECT | m_uTextStyle & ~DT_RIGHT & ~DT_CENTER);
					}
	...
	void LabelContainer::DoEvent(TEvent& event)
	{
	...
	void LabelContainer::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
	...
	void LabelContainer::PaintText(HDC hDC)
	{
	...
	void LabelContainer::SetShadowOffset(int _offset, int _angle)
	{
	...
	RectF LabelContainer::GetShadowOffset()
	{
		return m_ShadowOffset;
	}

	void LabelContainer::SetEnabledEffect(bool _EnabledEffect)
	{
	...
	bool LabelContainer::GetEnabledEffect()
	{
		return m_EnableEffect;
	}

	void LabelContainer::SetEnabledLuminous(bool bEnableLuminous)
	{
		m_bEnableLuminous = bEnableLuminous;
		Invalidate();
	}

	bool LabelContainer::GetEnabledLuminous()
	{
		return m_bEnableLuminous;
	}

	void LabelContainer::SetLuminousFuzzy(float fFuzzy)
	{
	...
	float LabelContainer::GetLuminousFuzzy()
	{
		return m_fLuminousFuzzy;
	}

	void LabelContainer::SetTextColor1(DWORD _TextColor1)
	{
		m_dwTextColor1 = _TextColor1;
		Invalidate();
	}

	DWORD LabelContainer::GetTextColor1()
	{
		return m_dwTextColor1;
	}

	void LabelContainer::SetTextShadowColorA(DWORD _TextShadowColorA)
	{
		m_dwTextShadowColorA = _TextShadowColorA;
		Invalidate();
	}

	DWORD LabelContainer::GetTextShadowColorA()
	{
		return m_dwTextShadowColorA;
	}

	void LabelContainer::SetTextShadowColorB(DWORD _TextShadowColorB)
	{
		m_dwTextShadowColorB = _TextShadowColorB;
		Invalidate();
	}

	DWORD LabelContainer::GetTextShadowColorB()
	{
		return m_dwTextShadowColorB;
	}

	void LabelContainer::SetGradientAngle(int _SetGradientAngle)
	{
		m_GradientAngle = _SetGradientAngle;
		Invalidate();
	}

	int LabelContainer::GetGradientAngle()
	{
		return m_GradientAngle;
	}

	void LabelContainer::SetEnabledStroke(bool _EnabledStroke)
	{
		m_EnabledStroke = _EnabledStroke;
		Invalidate();
	}

	bool LabelContainer::GetEnabledStroke()
	{
		return m_EnabledStroke;
	}

	void LabelContainer::SetStrokeColor(DWORD _StrokeColor)
	{
		m_dwStrokeColor = _StrokeColor;
		Invalidate();
	}

	DWORD LabelContainer::GetStrokeColor()
	{
		return m_dwStrokeColor;
	}

	void LabelContainer::SetEnabledShadow(bool _EnabledShadowe)
	{
		m_EnabledShadow = _EnabledShadowe;
		Invalidate();
	}

	bool LabelContainer::GetEnabledShadow()
	{
		return m_EnabledShadow;
	}

	void LabelContainer::SetGradientLength(int _GradientLength)
	{
		m_GradientLength = _GradientLength;
		Invalidate();
	}

	int LabelContainer::GetGradientLength()
	{

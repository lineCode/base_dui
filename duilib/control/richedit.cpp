...
DuiPoint RichEdit::GetCharPos(long lChar) const
{ 
    DuiPoint pt; 
...
DuiPoint RichEdit::PosFromChar(UINT nChar) const
{ 
    POINTL pt; 
    TxSendMessage(EM_POSFROMCHAR, (WPARAM)&pt, nChar, 0); 
    return DuiPoint(pt.x, pt.y); 
}

int RichEdit::CharFromPos(DuiPoint pt) const
{ 
...
void RichEdit::DoEvent(Event& event)
{
...
{
    //return DuiSize(m_rcItem); // 这种方式在第一次设置大小之后就大小不变了
    return ScrollBox::EstimateSize(szAvailable);
}
...
{
	ScrollBox::Move(szOffset, bNeedInvalidate);
	if( m_pTwh != NULL ) {
		RECT rc = m_rcItem;
...
bool RichEdit::DoPaint(HDC hDC, const RECT& rcPaint, Control* pStopControl)
{
	...
	if( m_pTwh && m_pTwh->IsShowCaret() && m_pManager->IsLayered() && IsFocused() && m_bDrawCaret ) {
		...
		if( ::PtInRect(&m_rcItem, ptCaret) ) {
			...
			Render::DrawLine(hDC, rcCaret, m_pTwh->GetCaretWidth(), 0xFF000000);
		}
	}
...
void RichEdit::InsertHBitmap(InsertCustomItemErrorCallback callback, HBITMAP hbitmap, bool custom)
{
	...
	String tmp_image_path = UIManager::GetInstancePath() + L"tmp\\re\\";

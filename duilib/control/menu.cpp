					...
					DWORD dwAlignment/* = eMenuAlignment_Left | eMenuAlignment_Top*/, bool isLayeredWindow/* = true*/)
{
	...
	Create(NULL, L"MainTrayMenu", WS_POPUP , WS_EX_TOOLWINDOW | WS_EX_TOPMOST, DuiRect(0,0,152,80));	
#else
	Create((m_pOwner == NULL) ? pMainPaintManager->GetPaintWindow() : m_pOwner->GetManager()->GetPaintWindow(), NULL, WS_POPUP , WS_EX_TOOLWINDOW | WS_EX_TOPMOST, DuiRect());
#endif
}

void CMenuWnd::Notify(Event& msg)
{
...
void MenuElement::DrawItemIcon(HDC hDC, const RECT& rcItem)
{
	if ( m_strIcon != _T("") )
	{
		if (!(m_bCheckItem && !GetChecked()))
		{
			...
			Render::DrawImage(hDC, m_pManager, m_rcItem, m_rcPaint, drawinfo);
		}			
	}
}
...
SIZE MenuElement::EstimateSize(SIZE szAvailable)
{
	...
	if(cXY.cy == 0) {
		...
		if( pInfo->bShowHtml ) {   
			int nLinks = 0;
			Render::DrawHtmlText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText.c_str(), iTextColor, NULL, NULL, nLinks, pInfo->nFont, DT_CALCRECT | pInfo->uTextStyle);
		}
		else {
			Render::DrawText(m_pManager->GetPaintDC(), m_pManager, rcText, m_sText.c_str(), iTextColor, pInfo->nFont, DT_CALCRECT | pInfo->uTextStyle);
		}
...
#endif
void MenuElement::DoEvent(Event& event)
{
...
void MenuElement::CreateMenuWnd()
{
	...
	ASSERT(m_pWindow);
	m_pWindow->Init(static_cast<MenuElement*>(this), _T(""), _T(""), DuiPoint(), NULL);
}

...
namespace dui
{
	class EditWnd : public CWindowWnd
	{
	public:
		EditWnd();
	...
	LRESULT EditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if( uMsg == WM_CREATE ) {
			...
			m_pOwner->GetManager()->SendNotify(m_pOwner, UIEVENT_RETURN);
		}
			...
			if (clrColor < 0xFF000000) {
				...
				HBITMAP hBmpEditBk = Render::GenerateBitmap(m_pOwner->GetManager(), rcWnd, m_pOwner, clrColor);
			...
			if (m_pOwner->GetManager()->IsLayered()) {
				...
				if( m_pOwner->IsEnabled() && m_bDrawCaret ) { // todo:ÅÐ¶ÏÊÇ·ñenabled
					...
					Render::DrawLine((HDC)wParam, rcCaret, 1, 0xFF000000);
				}
				return lRes;
			}
			bHandled = FALSE;
	...
	LRESULT EditWnd::OnEditChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		m_pOwner->GetManager()->SendNotify(m_pOwner, UIEVENT_TEXTCHANGE);
	...
	{
		SetTextPadding(DuiRect(4, 3, 4, 3));
	...
	void Edit::PaintText(HDC hDC)
	{
		...
		if( IsEnabled() ) {
			Render::DrawText(hDC, m_pManager, rc, sText.c_str(), m_dwTextColor, \
				m_iFont, DT_SINGLELINE | m_uTextStyle);
		}
		else {
			Render::DrawText(hDC, m_pManager, rc, sText.c_str(), m_dwDisabledTextColor, \
				m_iFont, DT_SINGLELINE | m_uTextStyle);

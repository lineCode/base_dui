...
DWORD Render::AdjustColor(DWORD dwColor, short H, short S, short L)
{
...
HBITMAP Render::CreateARGB32Bitmap(HDC hDC, int cx, int cy, COLORREF** pBits)
{
	LPBITMAPINFO lpbiSrc = NULL;
	lpbiSrc = (LPBITMAPINFO) new BYTE[sizeof(BITMAPINFOHEADER)];
...
ImageInfo* Render::LoadImage(STRINGorID bitmap, UIManager* pManager, LPCTSTR type, DWORD mask)
{
	...
	do 
	{
		if( type == NULL ) {
			...
			else  sFile = UIManager::GetGlobalResDir();
			...
			HRSRC hResource = ::FindResource(UIManager::GetResourceDll(), bitmap.m_lpstr, type);
			...
			dwSize = ::SizeofResource(UIManager::GetResourceDll(), hResource);
			if( dwSize == 0 ) break;
			pData = new BYTE[ dwSize ];
	...
	if (!pData)
	{
		::MessageBox(0, _T("Render::LoadImage CreateFile err！"), _T("抓BUG"), MB_OK);
		return NULL;
	}
    ...
    if (!type || _tcscmp(type, RES_TYPE_COLOR) != 0) {
        ...
        delete[] pData;
        if( !pImage ) {
			::MessageBox(0, _T("Render::LoadImage stbi_load_from_memory err！"), _T("抓BUG"), MB_OK);
            return NULL;
        }
    }
    BITMAPINFO bmi;
	...
	if( !hBitmap ) {
		::MessageBox(0, _T("Render::LoadImage CreateDIBSection err！"), _T("抓BUG"), MB_OK);
		return NULL;
	}
...
void Render::DrawImage(HDC hDC, HBITMAP hBitmap, const RECT& rc, const RECT& rcPaint,
...
bool Render::DrawImage(HDC hDC, UIManager* pManager, const RECT& rcItem, const RECT& rcPaint, 
					  DrawInfo& drawInfo)
{
	// 1、aaa.jpg
	...
	if( drawInfo.pImageInfo == NULL ) {
		...
		while( *pstrImage != _T('\0') ) {
			...
			if( !sValue.empty() ) {
				if( sItem == _T("file") ) {
					...
					drawInfo.rcDestOffset.left = _tcstol(sValue.c_str(), &pstr, 10);  ASSERT(pstr);    
					...
					drawInfo.rcBmpPart.left = _tcstol(sValue.c_str(), &pstr, 10);  ASSERT(pstr);    
					...
					drawInfo.rcScale9.left = _tcstol(sValue.c_str(), &pstr, 10);  ASSERT(pstr);    
					...
					drawInfo.uFade = (BYTE)_tcstoul(sValue.c_str(), &pstr, 10);
				}
				else if( sItem == _T("hole") ) {
					drawInfo.bHole = (_tcscmp(sValue.c_str(), _T("true")) == 0);
				}
				else if( sItem == _T("xtiled") ) {
					drawInfo.bTiledX = (_tcscmp(sValue.c_str(), _T("true")) == 0);
				}
				else if( sItem == _T("ytiled") ) {
					drawInfo.bTiledY = (_tcscmp(sValue.c_str(), _T("true")) == 0);
				}
				else if( sItem == _T("hsl") ) {
					bUseHSL = (_tcscmp(sValue.c_str(), _T("true")) == 0);
				}
			}
			if( *pstrImage++ != _T(' ') ) break;
		}
		...
		if( bUseRes == false ) {
			...
			data = pManager->GetImageEx((LPCTSTR)sImageName.c_str(), (LPCTSTR)sImageResType.c_str(), dwMask, bUseHSL);
		}
...
void Render::DrawColor(HDC hDC, const RECT& rc, DWORD color)
{
...
void Render::DrawGradient(HDC hDC, const RECT& rc, DWORD dwFirst, DWORD dwSecond, bool bVertical, int nSteps)
{
    typedef BOOL (WINAPI *LPALPHABLEND)(HDC, int, int, int, int,HDC, int, int, int, int, BLENDFUNCTION);
    static LPALPHABLEND lpAlphaBlend = (LPALPHABLEND) ::GetProcAddress(::GetModuleHandle(_T("msimg32.dll")), "AlphaBlend");
    if( lpAlphaBlend == NULL ) lpAlphaBlend = AlphaBitBlt;
...
void Render::DrawLine( HDC hDC, const RECT& rc, int nSize, DWORD dwPenColor, int nStyle)
{
	ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
...
void Render::DrawRect(HDC hDC, const RECT& rc, int nSize, DWORD dwPenColor, int nStyle)
{
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
...
void Render::DrawRoundRect(HDC hDC, const RECT& rc, int nSize, int width, int height, DWORD dwPenColor, int nStyle)
{
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
    HPEN hPen = ::CreatePen(nStyle | PS_INSIDEFRAME, nSize, RGB(GetBValue(dwPenColor), GetGValue(dwPenColor), GetRValue(dwPenColor)));
    HPEN hOldPen = (HPEN)::SelectObject(hDC, hPen);
...
void Render::DrawText(HDC hDC, UIManager* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, int iFont, UINT uStyle)
{
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
    if( pstrText == NULL || pManager == NULL ) return;

	String sText = pstrText;
	UIManager::ProcessMultiLanguageTokens(sText);
...
void Render::DrawHtmlText(HDC hDC, UIManager* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, String* sLinks, int& nLinkRects, int iDefaultFont, UINT uStyle)
{
            ...
            case _T('a'):  // Link
                {
                    ...
                    if( pFontInfo->bUnderline == false ) {
						...
						if( hFont == NULL ) {
							hFont = pManager->AddFont(g_iFontID, pFontInfo->sFontName.c_str(), pFontInfo->iSize, pFontInfo->bBold, true, pFontInfo->bItalic);
							g_iFontID += 1;
						}
            ...
            case _T('b'):  // Bold
                {
                    ...
                    if( pFontInfo->bBold == false ) {
						...
						if( hFont == NULL ) {
							hFont = pManager->AddFont(g_iFontID, pFontInfo->sFontName.c_str(), pFontInfo->iSize, true, pFontInfo->bUnderline, pFontInfo->bItalic);
							g_iFontID += 1;
						}
                        pFontInfo = pManager->GetFontInfo(hFont);
                        aFontArray.Add(pFontInfo);
                ...
                {    
                    ...
                    if( sName.empty() ) { // Italic
                        ...
                        if( pFontInfo->bItalic == false ) {
							...
							if( hFont == NULL ) {
								hFont = pManager->AddFont(g_iFontID, pFontInfo->sFontName.c_str(), pFontInfo->iSize, pFontInfo->bBold, pFontInfo->bUnderline, true);
								g_iFontID += 1;
							}
                            pFontInfo = pManager->GetFontInfo(hFont);
                            aFontArray.Add(pFontInfo);
						...
						if( _tcsstr(sImageString.c_str(), _T("file=\'")) != NULL || _tcsstr(sImageString.c_str(), _T("res=\'")) != NULL ) {
							...
							pImageInfo = pManager->GetImageEx((LPCTSTR)sImageName.c_str(), sImageResType.c_str());
						}
						else
							pImageInfo = pManager->GetImageEx((LPCTSTR)sName.c_str());
						...
						if( pImageInfo ) {
                            ...
                            if( pt.x + iWidth > rc.right && pt.x > rc.left && (uStyle & DT_SINGLELINE) == 0 ) {
                                ...
                                pstrNextStart = NULL;
                                if( bDraw && bLineDraw ) {
                                    DuiRect rcImage(pt.x + cxOffset, pt.y + cyLineHeight - iHeight, pt.x + + cxOffset + iWidth, pt.y + cyLineHeight);
									iVAlign = DT_BOTTOM;
...
HBITMAP Render::GenerateBitmap(UIManager* pManager, RECT rc, Control* pStopControl, DWORD dwFilterColor)
{
...
HBITMAP Render::GenerateBitmap(UIManager* pManager, Control* pControl, RECT rc, DWORD dwFilterColor)
{
...
SIZE Render::GetTextSize( HDC hDC, UIManager* pManager , LPCTSTR pstrText, int iFont, UINT uStyle )
{
	String sText = pstrText;
	UIManager::ProcessMultiLanguageTokens(sText);
	pstrText = sText.c_str();

...
UIManager* Control::GetManager() const
{
    return m_pManager;
}

void Control::SetManager(UIManager* pManager, Control* pParent, bool bInit)
{
...
bool Control::DrawImage(HDC hDC, DrawInfo& drawInfo)
{
	return Render::DrawImage(hDC, m_pManager, m_rcItem, m_rcPaint, drawInfo);
}
...
RECT Control::GetRelativePos() const
{
	...
	if( pParent != NULL ) {
		...
		DuiRect rcRelativePos(m_rcItem);
...
DWORD Control::GetAdjustColor(DWORD dwColor)
{
    ...
    short H, S, L;
    UIManager::GetHSL(&H, &S, &L);
    return Render::AdjustColor(dwColor, H, S, L);
}
...
void Control::DoEvent(Event& event)
{
...
bool Control::DoPaint(HDC hDC, const RECT& rcPaint, Control* pStopControl)
{
	...
	if (cxyBorderRound.cx > 0 || cxyBorderRound.cy > 0) {
		...
		RenderClip::GenerateRoundClip(hDC, m_rcPaint, m_rcItem, cxyBorderRound.cx, cxyBorderRound.cy, roundClip);
        PaintBkColor(hDC);
        PaintBkImage(hDC);
...
{
    if( m_dwBackColor != 0 ) {
        if( m_dwBackColor >= 0xFF000000 ) Render::DrawColor(hDC, m_rcPaint, GetAdjustColor(m_dwBackColor));
        else Render::DrawColor(hDC, m_rcItem, GetAdjustColor(m_dwBackColor));
    }
}
	...
	if(rcBorderSize.left > 0 && (m_dwBorderColor != 0 || m_dwFocusBorderColor != 0)) {
		if( cxyBorderRound.cx > 0 || cxyBorderRound.cy > 0 )//画圆角边框
		{
			if (IsFocused() && m_dwFocusBorderColor != 0)
				Render::DrawRoundRect(hDC, m_rcItem, rcBorderSize.left, cxyBorderRound.cx, cxyBorderRound.cy, GetAdjustColor(m_dwFocusBorderColor), m_nBorderStyle);
			else
				Render::DrawRoundRect(hDC, m_rcItem, rcBorderSize.left, cxyBorderRound.cx, cxyBorderRound.cy, GetAdjustColor(m_dwBorderColor), m_nBorderStyle);
		}
		else {
			if (rcBorderSize.right == rcBorderSize.left && rcBorderSize.top == rcBorderSize.left && rcBorderSize.bottom == rcBorderSize.left) {
				if (IsFocused() && m_dwFocusBorderColor != 0)
					Render::DrawRect(hDC, m_rcItem, rcBorderSize.left, GetAdjustColor(m_dwFocusBorderColor), m_nBorderStyle);
				else
					Render::DrawRect(hDC, m_rcItem, rcBorderSize.left, GetAdjustColor(m_dwBorderColor), m_nBorderStyle);
			}
			else {
				RECT rcBorder;
				if(rcBorderSize.left > 0){
					...
					if (IsFocused() && m_dwFocusBorderColor != 0)
						Render::DrawLine(hDC,rcBorder,rcBorderSize.left,GetAdjustColor(m_dwFocusBorderColor),m_nBorderStyle);
					else
						Render::DrawLine(hDC,rcBorder,rcBorderSize.left,GetAdjustColor(m_dwBorderColor),m_nBorderStyle);
				}
				if(rcBorderSize.top > 0) {
					...
					if (IsFocused() && m_dwFocusBorderColor != 0)
						Render::DrawLine(hDC,rcBorder,rcBorderSize.top,GetAdjustColor(m_dwFocusBorderColor),m_nBorderStyle);
					else
						Render::DrawLine(hDC,rcBorder,rcBorderSize.top,GetAdjustColor(m_dwBorderColor),m_nBorderStyle);
				}
				if(rcBorderSize.right > 0) {
                    ...
                    rcBorder.right  = rcBorder.left;
					if (IsFocused() && m_dwFocusBorderColor != 0)
						Render::DrawLine(hDC,rcBorder,rcBorderSize.right,GetAdjustColor(m_dwFocusBorderColor),m_nBorderStyle);
					else
						Render::DrawLine(hDC,rcBorder,rcBorderSize.right,GetAdjustColor(m_dwBorderColor),m_nBorderStyle);
				}
				if(rcBorderSize.bottom > 0) {
                    ...
                    rcBorder.right -= rcBorderSize.right;
					if (IsFocused() && m_dwFocusBorderColor != 0)
						Render::DrawLine(hDC,rcBorder,rcBorderSize.bottom,GetAdjustColor(m_dwFocusBorderColor),m_nBorderStyle);
					else
						Render::DrawLine(hDC,rcBorder,rcBorderSize.bottom,GetAdjustColor(m_dwBorderColor),m_nBorderStyle);
				}
			}
		}
	}

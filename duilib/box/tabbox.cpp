...
namespace dui
{
	TabBox::TabBox() : m_iCurSel(-1), m_uFadeSwitchPos(0), m_uFadeSwitchDelta(0), m_cFadeSwitchLeftId(-1), m_cFadeSwitchRightId(-1), m_bFadeSwitchRightToLeft(false)/*, pOldFadeSwitchItem_(NULL)*/
	{
	}

	LPCTSTR TabBox::GetClass() const
	{
		return DUI_CTR_TABBOX;
	}

	LPVOID TabBox::GetInterface(LPCTSTR pstrName)
	{
		if (_tcscmp(pstrName, DUI_CTR_TABBOX) == 0) return static_cast<TabBox*>(this);
		return ScrollBox::GetInterface(pstrName);
	}
	...
	{
		bool ret = ScrollBox::Add(pControl);
		if( !ret ) return ret;
	...
	{
		bool ret = ScrollBox::AddAt(pControl, iIndex);
		if( !ret ) return ret;
	...
	bool TabBox::Remove(Control* pControl, bool bDoNotDestroy)
	{
		if( pControl == NULL) return false;

		int index = GetItemIndex(pControl);
		bool ret = ScrollBox::Remove(pControl, bDoNotDestroy);
		if( !ret ) return false;
	...
	bool TabBox::SelectItem(int iIndex,  bool bTriggerEvent)
	{
		...
		if (GetFadeSwitch() /*&& iOldSel >= 0*/) {
			...
			//printf("TabBox SetTimer\n");
		}
		return true;
	}

	bool TabBox::SelectItem(Control* pControl, bool bTriggerEvent)
	{
	...
	void TabBox::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else return ScrollBox::SetAttribute(pstrName, pstrValue);
	}
	...
	void TabBox::DoEvent(Event& event)
	{
		bool bHandle = false;
	...
	bool TabBox::DoPaint(HDC hDC, const RECT& rcPaint, Control* pStopControl)
	{
#if 1
		RECT rcTemp = { 0 };

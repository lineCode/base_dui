...
namespace dui
{
	TileBox::TileBox() : m_nColumns(1), m_nRows(0), m_nColumnsFixed(0), m_iChildVPadding(0),
		m_bIgnoreItemPadding(true)
	{
		m_szItem.cx = m_szItem.cy = 80;
	}

	LPCTSTR TileBox::GetClass() const
	{
		return DUI_CTR_TILEBOX;
	}

	LPVOID TileBox::GetInterface(LPCTSTR pstrName)
	{
		if (_tcscmp(pstrName, DUI_CTR_TILEBOX) == 0) return static_cast<TileBox*>(this);
		return ScrollBox::GetInterface(pstrName);
	}

	int TileBox::GetFixedColumns() const
	{
		return m_nColumnsFixed;
	}

	void TileBox::SetFixedColumns(int iColums)
	{
		if( iColums < 0 ) return;
	...
	int TileBox::GetChildVPadding() const
	{
		return m_iChildVPadding;
	}

	void TileBox::SetChildVPadding(int iPadding)
	{
	...
	void TileBox::SetItemSize(SIZE szSize)
	{
	...
	int TileBox::GetRows() const
	{
		return m_nRows;
	}

	void TileBox::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else ScrollBox::SetAttribute(pstrName, pstrValue);
	}

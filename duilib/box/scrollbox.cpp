...
namespace dui
{
	...
	LPCTSTR ScrollBox::GetClass() const
	{
		return DUI_CTR_SCROLLBOX;
	}

	LPVOID ScrollBox::GetInterface(LPCTSTR pstrName)
	{
		/*if (_tcscmp(pstrName, DUI_CTR_ICONTAINER) == 0) return static_cast<IContainer*>(this);
		else */if (_tcscmp(pstrName, DUI_CTR_SCROLLBOX) == 0) return static_cast<ScrollBox*>(this);
	...
	void ScrollBox::SetScrollBarFloat(bool bFloat)
	{
	...
	SIZE ScrollBox::GetScrollPos() const
	{
		SIZE sz = { 0, 0 };
	...
	SIZE ScrollBox::GetScrollRange() const
	{
		SIZE sz = { 0, 0 };
	...
	void ScrollBox::SetScrollPos(SIZE szPos)
	{
		int cx = 0;
		int cy = 0;
	...
	RECT ScrollBox::GetClientPos() const
	{
		RECT rc = m_rcItem;
	...
	void ScrollBox::SetPos(RECT rc, bool bNeedInvalidate)
	{
#if 1
	...
	void ScrollBox::Move(SIZE szOffset, bool bNeedInvalidate)
	{
	...
	void ScrollBox::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
	...
	void ScrollBox::SetManager(UIManager* pManager, Control* pParent, bool bInit)
	{
	...
	Control* ScrollBox::FindControl(FINDCONTROLPROC Proc, LPVOID pData, UINT uFlags)
	{
	...
	bool ScrollBox::DoPaint(HDC hDC, const RECT& rcPaint, Control* pStopControl)
	{
		RECT rcTemp = { 0 };
	...
	void ScrollBox::ProcessScrollBar(RECT rc, int cxRequired, int cyRequired)
	{
		if (m_pHorizontalScrollBar != NULL) {
			if (m_pVerticalScrollBar == NULL) {

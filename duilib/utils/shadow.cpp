...
void ShadowUI::Create(UIManager* pPaintManager)
{
	if(!m_bIsShowShadow)
		return;

	// Already initialized
	_ASSERT(UIManager::GetInstance() != INVALID_HANDLE_VALUE);
	...
	m_hWnd = CreateWindowEx(WS_EX_LAYERED | WS_EX_TRANSPARENT, strWndClassName, NULL,
		...
		CW_USEDEFAULT, 0, 0, 0, hParentWnd, NULL, UIManager::GetInstance(), NULL);
...
#pragma warning(disable: 4311)	// temporrarily disable the type_cast warning in Win32
	SetWindowLongPtr(hParentWnd, GWLP_WNDPROC, (LONG_PTR)ShadowUI::ParentProc);
...
void ShadowUI::Update(HWND hParent)
{
	...
	if (m_bIsImageMode)
	{
		...
		Render::DrawImage(hMemDC, data->hBitmap, rcPaint, rcPaint, rcBmpPart, m_rcShadowCorner, data->bAlpha, 0xFF, true, false, false);
	}
	else
	{
...
bool ShadowUI::SetImage(LPCTSTR szImage)
{
	if (szImage == NULL)
		return false;

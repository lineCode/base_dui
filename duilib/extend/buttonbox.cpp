...
namespace dui{
	...
	LPCTSTR ButtonBox::GetClass() const
	{
		return DUI_CTR_BUTTONBOX;
	}

	LPVOID ButtonBox::GetInterface(LPCTSTR pstrName)
	{
		if (_tcscmp(pstrName, DUI_CTR_BUTTONBOX) == 0) return static_cast<ButtonBox*>(this);
		return __super::GetInterface(pstrName);
	}

	UINT ButtonBox::GetControlFlags() const
	{
		return (IsKeyboardEnabled() ? UIFLAG_TABSTOP : 0) | (IsEnabled() ? UIFLAG_SETCURSOR : 0);
	}

	void ButtonBox::DoEvent(Event& event)
	{
	...
	void ButtonBox::SetEnabled(bool bEnable)
	{
	...
	void ButtonBox::SetHotBkColor(DWORD dwColor)
	{
		m_dwHotBkColor = dwColor;
	}

	DWORD ButtonBox::GetHotBkColor() const
	{
		return m_dwHotBkColor;
	}

	void ButtonBox::SetHotTextColor(DWORD dwColor)
	{
		m_dwHotTextColor = dwColor;
	}

	DWORD ButtonBox::GetHotTextColor() const
	{
		return m_dwHotTextColor;
	}

	void ButtonBox::SetPushedTextColor(DWORD dwColor)
	{
		m_dwPushedTextColor = dwColor;
	}

	DWORD ButtonBox::GetPushedTextColor() const
	{
		return m_dwPushedTextColor;
	}

	void ButtonBox::SetFocusedTextColor(DWORD dwColor)
	{
		m_dwFocusedTextColor = dwColor;
	}

	DWORD ButtonBox::GetFocusedTextColor() const
	{
		return m_dwFocusedTextColor;
	}

	LPCTSTR ButtonBox::GetNormalImage()
	{
		return m_diNormal.sDrawString.c_str();
	}

	void ButtonBox::SetNormalImage(LPCTSTR pStrImage)
	{
	...
	LPCTSTR ButtonBox::GetHotImage()
	{
		return m_diHot.sDrawString.c_str();
	}

	void ButtonBox::SetHotImage(LPCTSTR pStrImage)
	{
	...
	LPCTSTR ButtonBox::GetPushedImage()
	{
		return m_diPushed.sDrawString.c_str();
	}

	void ButtonBox::SetPushedImage(LPCTSTR pStrImage)
	{
	...
	LPCTSTR ButtonBox::GetFocusedImage()
	{
		return m_diFocused.sDrawString.c_str();
	}

	void ButtonBox::SetFocusedImage(LPCTSTR pStrImage)
	{
	...
	LPCTSTR ButtonBox::GetDisabledImage()
	{
		return m_diDisabled.sDrawString.c_str();
	}

	void ButtonBox::SetDisabledImage(LPCTSTR pStrImage)
	{
	...
	LPCTSTR ButtonBox::GetHotForeImage()
	{
		return m_diHotFore.sDrawString.c_str();
	}

	void ButtonBox::SetHotForeImage(LPCTSTR pStrImage)
	{
	...
	void ButtonBox::SetFiveStatusImage(LPCTSTR pStrImage)
	{
		m_diNormal.Clear();
		m_diNormal.sDrawString = pStrImage;
		...
		if (m_diPushed.pImageInfo) {
			...
			m_diPushed.rcBmpPart = DuiRect(width, 0, width * 2, height);
		}
		...
		if (m_diHot.pImageInfo) {
			...
			m_diHot.rcBmpPart = DuiRect(width * 2, 0, width * 3, height);
		}
		...
		if (m_diFocused.pImageInfo) {
			...
			m_diFocused.rcBmpPart = DuiRect(width * 3, 0, width * 4, height);
		}
		...
		if (m_diDisabled.pImageInfo) {
			...
			m_diDisabled.rcBmpPart = DuiRect(width * 4, 0, width * 5, height);
		}

		Invalidate();
	}

	void ButtonBox::SetFadeAlphaDelta(BYTE uDelta)
	{
		m_uFadeAlphaDelta = uDelta;
	}

	BYTE ButtonBox::GetFadeAlphaDelta()
	{
		return m_uFadeAlphaDelta;
	}

	SIZE ButtonBox::EstimateSize(SIZE szAvailable)
	{
	...
	void ButtonBox::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
	...
	void ButtonBox::PaintText(HDC hDC)
	{
		...
		if (m_bShowHtml)
			Render::DrawHtmlText(hDC, m_pManager, rc, m_sText.c_str(), clrColor, \
			NULL, NULL, nLinks, m_iFont, m_uTextStyle);
		else
			Render::DrawText(hDC, m_pManager, rc, m_sText.c_str(), clrColor, \
			m_iFont, m_uTextStyle);
	}

	void ButtonBox::PaintStatusImage(HDC hDC)
	{
		if (IsFocused()) m_uButtonState |= UISTATE_FOCUSED;
		else m_uButtonState &= ~UISTATE_FOCUSED;
		if (!IsEnabled()) m_uButtonState |= UISTATE_DISABLED;
		...
		if ((m_uButtonState & UISTATE_DISABLED) != 0) {
			...
			if (DrawImage(hDC, m_diHotFore)) return;
				...
				Render::DrawColor(hDC, m_rcPaint, GetAdjustColor(m_dwHotBkColor));
				return;
			}
			else goto Label_ForeImage;
		}

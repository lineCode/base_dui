...
//run in ui theard 
void MediaPlayer::BitbltCallback(HBITMAP hbmp, int width, int height)
{
	if (m_bStoped)		//关闭程序时 有可能这个任务还在ui线程最后，MediaPlayer先析构了，引起崩溃
	{
		printf("MediaPlayer::BitbltCallback m_bStoped = true\n");
		return;
	}
    ...
    if (m_pManager)
    {
        ...
        if (m_pScreen)
            m_pScreen->Paint(hdc, m_pScreen->GetPos());
        //Render::DrawImage(hdc, m_pManager, m_rcItem, /*m_rcItem*/m_rcPaint, m_diCurrFrame);
        /*printf("m_rcItem{%d,%d,%d,%d}, m_rcPaint{%d,%d,%d,%d}\n", m_rcItem.left, m_rcItem.top, m_rcItem.right, m_rcItem.bottom,
            m_rcPaint.left, m_rcPaint.top, m_rcPaint.right, m_rcPaint.bottom);*/
        /*printf("m_pBoxSlider{%d,%d,%d,%d}\n", m_pBoxSlider->GetPos().left, m_pBoxSlider->GetPos().top, m_pBoxSlider->GetPos().right, m_pBoxSlider->GetPos().bottom);*/
        if (m_pBoxSlider)
            m_pBoxSlider->Paint(hdc, m_pBoxSlider->GetPos());

	}
}

//run in video play theard 
void MediaPlayer::StartPlayCallback(std::string file)
{
    ...
    if (fmt_ctx->duration != AV_NOPTS_VALUE){
        ...
        shared::Post2UI(std::bind([this](int64_t duration){
            if (m_pSlider)
                m_pSlider->SetMaxValue(duration / AV_TIME_BASE);
            TCHAR chBuf[10] = {};
            int sec = duration / AV_TIME_BASE;
            _stprintf(chBuf, _T("%02d:%02d:%02d"), sec / 3600, (sec % 3600) / 60, sec % 60);
            if (m_pLbTime)
                m_pLbTime->SetText(chBuf);
        }, fmt_ctx->duration + 5000));
    }
		...
		if (pkt.stream_index == video_stream || end_of_stream) {
			...
			if (got_frame) {
				...
				if (!hbmp)
				{
					...
					shared::Post2UI(std::bind(&MediaPlayer::BitbltCallback, this, hbmp, dst_w, dst_h));
			}
			av_packet_unref(&pkt);
			av_init_packet(&pkt);
		}
...
void MediaPlayer::StartPlayRtspCallback(std::string url)
{
    assert(m_play_status == PS_STOP);
    m_bStoped = false;
    AVCodec *codec = NULL;
    ...
    result = avformat_open_input(&fmt_ctx, url.c_str(), NULL, NULL);
    if (result < 0) {
        av_log(NULL, AV_LOG_ERROR, "Can't open file\n");
        assert(0);
        goto err;
...
void MediaPlayer::afterGettingRtspFrame(char *buf, int size)
{
    /*printf("MediaPlayer::afterGettingRtspFrame");*/
    assert(m_play_status == PS_STOP);
    //m_bStoped = false;
    ...
    AVIOContext *avio = avio_alloc_context(iobuffer, 32768, 0, NULL, nullptr, NULL, NULL);
    ic->pb = avio;
